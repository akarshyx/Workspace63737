...
from styled_buttons import StyledInlineKeyboardButton, danger_btn, primary_btn, success_btn
...
def _number_button(number: int, session: dict[str, Any]):
    ...
    if hit:
        return success_btn(str(number), callback_data=f"{KENO_CALLBACK_PREFIX}noop:{session_id}")
    if revealed:
        return danger_btn(str(number), callback_data=f"{KENO_CALLBACK_PREFIX}noop:{session_id}")
    if selected:
        return primary_btn(str(number), callback_data=callback)
    if session.get("status") != "setup":
        return StyledInlineKeyboardButton(
            str(number),
            callback_data=f"{KENO_CALLBACK_PREFIX}noop:{session_id}",
        )
    return StyledInlineKeyboardButton(str(number), callback_data=callback)
...
def _render_mode_picker(session: dict[str, Any]) -> tuple[str, InlineKeyboardMarkup]:
    ...
    for key in ("classic", "easy", "medium", "hard"):
        ...
        factory = success_btn if key == mode_key else StyledInlineKeyboardButton
        mode_buttons.append(factory(button_text, callback_data=f"{KENO_CALLBACK_PREFIX}mode:{session['session_id']}:{key}"))
    keyboard = [
        mode_buttons[:2],
        mode_buttons[2:],
        [success_btn("Open number board", callback_data=f"{KENO_CALLBACK_PREFIX}open:{session['session_id']}")],
        [danger_btn("Back", callback_data=f"{KENO_CALLBACK_PREFIX}back:{session['session_id']}")],
    ]
    text = (
        "🎯 <b>KENO</b>\n\n"
        "Choose your risk mode, then select 1–10 numbers from the board.\n\n"
        f"<b>Bet amount:</b> {service.format_balance(session['bet_amount'], currency)}\n"
        f"<b>Mode:</b> {_mode_label(mode_key)}\n"
        f"<b>Balance:</b> {service.format_balance(service.get_balance(user_id), currency)}\n\n"
        "<i>The draw is 10 unique numbers from 1–40. "
        "Payouts are generated from the exact odds and the selected mode's RTP.</i>"
    )
...
def _render_board(session: dict[str, Any]) -> tuple[str, InlineKeyboardMarkup]:
    ...
    status = session.get("status")
    if status == "revealing":
        status_line = "Numbers are being revealed…"
    elif status == "finished":
        status_line = "Round complete"
    else:
        status_line = "Select 1–10 numbers, then place your bet."

    text = (
        "🎯 <b>KENO</b>\n\n"
        f"<b>Mode:</b> {_mode_label(session['mode'])}\n"
        f"<b>Bet:</b> {service.format_balance(session['bet_amount'], currency)}\n"
        f"<b>Selected:</b> {spots}/{KENO_CONFIG['maximum_selection']}\n"
        f"<b>Hits:</b> {hits}/{spots or 0}\n"
    )
    if status == "revealing":
        current = float(session.get("current_multiplier", 0.0))
        potential = float(session.get("potential_win", 0.0))
        text += (
            f"<b>Multiplier:</b> {current:.2f}x\n"
            f"<b>Potential win:</b> {service.format_balance(potential, currency)}\n"
        )
    elif status == "finished":
        final = float(session.get("current_multiplier", 0.0))
        payout = float(session.get("final_payout", 0.0))
        text += (
            f"<b>Multiplier:</b> {final:.2f}x\n"
            f"<b>Won:</b> {service.format_balance(payout, currency)}\n"
        )
    text += f"\n{status_line}\n\n"
    text += f"<b>Payouts for {spots} spot(s):</b>\n{_format_table(spots, session['mode'])}"

    grid = []
    for start in range(1, KENO_CONFIG["number_count"] + 1, 8):
        grid.append([_number_button(n, session) for n in range(start, min(start + 8, KENO_CONFIG["number_count"] + 1))])

    if status == "setup":
        mode_buttons = []
        for key in ("classic", "easy", "medium", "hard"):
            label = _mode(key).label
            factory = success_btn if key == session["mode"] else StyledInlineKeyboardButton
            mode_buttons.append(factory(
                f"✓ {label}" if key == session["mode"] else label,
                callback_data=f"{KENO_CALLBACK_PREFIX}mode:{session['session_id']}:{key}",
            ))
        grid.append(mode_buttons[:2])
        grid.append(mode_buttons[2:])
        grid.append([
            primary_btn("1/2", callback_data=f"{KENO_CALLBACK_PREFIX}half:{session['session_id']}"),
            primary_btn("2x", callback_data=f"{KENO_CALLBACK_PREFIX}double:{session['session_id']}"),
        ])
        grid.append([
            primary_btn("Random Pick", callback_data=f"{KENO_CALLBACK_PREFIX}random:{session['session_id']}"),
            danger_btn("Clear Table", callback_data=f"{KENO_CALLBACK_PREFIX}clear:{session['session_id']}"),
        ])
        grid.append([success_btn("Bet", callback_data=f"{KENO_CALLBACK_PREFIX}bet:{session['session_id']}")])
        grid.append([danger_btn("Back", callback_data=f"{KENO_CALLBACK_PREFIX}back:{session['session_id']}")])
    elif status == "finished":
        grid.append([success_btn("Play Again", callback_data=f"{KENO_CALLBACK_PREFIX}again:{session['session_id']}")])
    else:
        grid.append([StyledInlineKeyboardButton("Round in progress", callback_data=f"{KENO_CALLBACK_PREFIX}noop:{session['session_id']}")])
...
async def _place_bet(query, context: ContextTypes.DEFAULT_TYPE, session: dict[str, Any]) -> None:
    ...
    selected.sort()
    min_spots = int(KENO_CONFIG["minimum_selection"])
    max_spots = int(KENO_CONFIG["maximum_selection"])
    if not (min_spots <= len(selected) <= max_spots):
        await _answer(query, "❌ Select at least 1 number before betting." if not selected else f"❌ Select no more than {max_spots} numbers.", show_alert=True)
        return
...
async def _settle_session(context: ContextTypes.DEFAULT_TYPE, session_id: str) -> None:
    ...
    try:
        text, markup = _render_board(session)
        text += (
            f"\n\n<b>Draw:</b> {', '.join(map(str, session['draw_numbers']))}\n"
            f"<b>Hit numbers:</b> {', '.join(map(str, session['hit_numbers'])) or 'None'}\n"
            f"<b>Balance:</b> {service.format_balance(service.get_balance(user_id), currency)}"
        )
        await context.bot.edit_message_text(
            chat_id=session["chat_id"],
        ...
        service.logger.warning("[KENO] Could not send final result for %s: %s", session_id, exc)
...
async def keno_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "back":
        ...
        try:
            ...
            await _edit(query, "Keno round cancelled.", InlineKeyboardMarkup([]))
        _svc().save()
        return
    ...
    if action == "open":
        ...
        return
    if action == "half":
        await _change_bet(query, session, 0.5)
        return
    if action == "double":
        await _change_bet(query, session, 2.0)
        return
    if action == "random":
    ...
    if action == "pick":
        ...
        if number in selected:
            ...
            await _answer(query, f"❌ You can select at most {KENO_CONFIG['maximum_selection']} numbers.", show_alert=True)
            return
        else:
